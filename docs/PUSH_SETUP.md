...
`/api/push-config` returns `{ key: null }` and the client never prompts — zero user-facing
...
## 3. What happens then
- The client fetches `/api/push-config`, sees a key, and (only for players who've already
  played, once, with a soft pre-prompt) offers "Daily reminder?". On accept it subscribes
  via the service worker and POSTs the subscription to `/api/push-subscribe` (stored in the
  Redis set `push:subs`).
...
```
curl -X POST "https://cinelinks.vercel.app/api/push-send" \
...
```json
"crons": [{ "path": "/api/push-send?secret=YOUR_PUSH_SECRET", "schedule": "0 9 * * *" }]
```
...
## 5. Endpoints
- `GET /api/push-config` → `{ key }` (public VAPID key or null).
- `POST /api/push-subscribe` `{ subscription }` → stores it. `DELETE` removes it.
- `POST /api/push-send` (needs `x-push-secret` or `?secret=`) → sends to all, prunes dead.

## Testing
After setting keys + redeploying: open the site (as a returning player), accept the prompt,
then call `/api/push-send` manually — you should get the notification. Check the response
`{ sent, pruned, total }`.
