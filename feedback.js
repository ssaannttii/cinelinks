...
(function () {
  ...
  function injectStyles() {
    ...
    s.textContent = [
      '#clFbBtn{position:fixed;right:14px;bottom:calc(58px + env(safe-area-inset-bottom));z-index:230;border:1px solid rgba(232,160,0,.52);border-radius:999px;background:rgba(37,44,53,.88);color:#f5c542;font:700 12px/1 Inter,-apple-system,sans-serif;padding:9px 13px;box-shadow:0 12px 30px rgba(0,0,0,.34);backdrop-filter:blur(10px);cursor:pointer}',
      '#clFbPanel{position:fixed;right:14px;bottom:calc(102px + env(safe-area-inset-bottom));z-index:231;width:min(310px,calc(100vw - 28px));display:none;border:1px solid rgba(255,255,255,.14);border-radius:16px;background:#2c343f;color:#f2f2f2;font-family:Inter,-apple-system,sans-serif;box-shadow:0 24px 70px rgba(0,0,0,.55);padding:14px}',
      ...
      '.clfb-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.clfb-choice{border:1px solid rgba(255,255,255,.12);border-radius:12px;background:rgba(255,255,255,.04);color:#eee;font:750 12px/1 Inter,-apple-system,sans-serif;padding:10px;cursor:pointer}.clfb-choice:hover{border-color:rgba(232,160,0,.5);color:#f5c542;background:rgba(232,160,0,.08)}',
      '.clfb-done{display:none;color:#7fd49a;font-weight:800;font-size:.82rem;margin-top:10px}.clfb-done.on{display:block}',
      '.clfb-beta{position:fixed;top:calc(10px + env(safe-area-inset-top));left:50%;transform:translateX(-50%);z-index:300;display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(245,197,66,.7);border-radius:999px;background:rgba(12,12,12,.88);color:#f5c542;font:800 12px/1 Inter,-apple-system,sans-serif;letter-spacing:.12em;text-transform:uppercase;padding:8px 13px;box-shadow:0 10px 28px rgba(0,0,0,.46),0 0 0 1px rgba(245,197,66,.18);backdrop-filter:blur(12px);pointer-events:none}',
