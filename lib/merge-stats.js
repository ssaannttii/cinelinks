...
(function (root, factory) {
  ...
  function mergeCollection(a, b) {
    ...
    // showcase vitrine: union preserving a's order first, capped at 6 slots
    var sca = Array.isArray(a.showcase) ? a.showcase : [], scb = Array.isArray(b.showcase) ? b.showcase : [];
    if (sca.length || scb.length) {
      var seen = {}, sc = [];
      sca.concat(scb).forEach(function (k) { if (!seen[k] && sc.length < 6) { seen[k] = 1; sc.push(k); } });
      out.showcase = sc;
    }
    // level-reward ladder: highest paid level wins (never re-pays)
    var lp = Math.max(a.lvlPaid | 0, b.lvlPaid | 0); if (lp) out.lvlPaid = lp;
    // daily-double: the newer day wins; same day → keep the furthest progress
    var add_ = a.dd, bdd = b.dd;
    if (add_ || bdd) {
      if (!add_) out.dd = bdd;
      else if (!bdd) out.dd = add_;
      else if (add_.d !== bdd.d) out.dd = (add_.d > bdd.d) ? add_ : bdd;
      else out.dd = { d: add_.d, n: Math.max(add_.n | 0, bdd.n | 0), done: (add_.done || bdd.done) ? 1 : 0 };
    }
    return out;
  }
